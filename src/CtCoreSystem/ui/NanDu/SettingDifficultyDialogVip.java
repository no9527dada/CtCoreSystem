...
//难度 激活版
public class SettingDifficultyDialogVip extends Dialog {

    public static Table container;
    Planet planet;
    Table current;
    public SettingDifficultyDialogVip(boolean 加载CT2) {
     ...
     public void addChangeDiffcutySlider(boolean 加载CT2) {
       ...
       //  Slider slider = !ActivateProgram.isActivated ? new Slider(1, 加载CT2 ? 6 : 4, 1, false) : new Slider(1, 加载CT2 ? 4 : 4, 1, false);
         ...
         Slider slider = 主动关闭激活 == true
                 ? new Slider(1, 加载CT2 ? 6 : 4, 1, false)
                 : new Slider(1, 4, 1, false);
         ...
         slider.addListener(new Tooltip(t -> {
             ...
             textContainer.add("激活版每次启动使用上次设置的难度").color(Color.white).pad(4); // 添加内边距
             t.add(textContainer);
         }));
         ...
         table.row();
         table.add( 主动关闭激活==false ?
                        加载CT2 ?toText("DcV.txt"):toText("Dc3.txt"):
                        加载CT2 ?toText("Dc2.txt"):toText("Dc3.txt")
                 )

                 .left().growX().wrap().width(400).maxWidth(400).pad(4).row();
