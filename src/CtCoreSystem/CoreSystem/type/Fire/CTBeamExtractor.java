package CtCoreSystem.CoreSystem.type.Fire;
...
//使用火种模组的代码  本人无法授权使用 找火种作者去商量
/** @see mindustry.world.blocks.production.Drill */
public class CTBeamExtractor extends mindustry.world.Block{
    ...
    public CTBeamExtractor(String name){
        ...
        barrels.add(new CTBeamExtractor.Barrel("", 0.0f, 0.0f, 4.0f, 1.0f, 1.0f));
    ...
    /** 在放置方块时调用，即玩家手持该方块准备建造时 */
    public void drawPlace(int x, int y, int rotation, boolean valid){
        super.drawPlace(x, y, rotation, valid);
        Drawf.dashCircle(wp(x), wp(y), range, Color.green);      // 显示最大采集范围
        Drawf.dashCircle(wp(x), wp(y), closeDst(), Color.red);   // 显示最小采集范围
        checkOre(x, y, true, null, null);        // 显示可开采资源
    }
    ...
    public class BeamExtractorBuild extends mindustry.gen.Building{
        ...
        public void updateTile(){
            ...
            if(drillTimer >= getDrillTime() && items.total() < itemCapacity){
                ...
                if (core != null && core.acceptItem(this, mining.drop())) {
                    core.handleItem(this, mining.drop());
                } else {
                    // 如果核心不可用，使用原有逻辑
                    offload(mining.drop());
                }
        ...
        public void offload(Item item) {
            // 尝试直接将物品输出到核心
            CoreBlock.CoreBuild core = Vars.state.teams.get(team()).core();
            if (core != null && core.acceptItem(this, item)) {
                core.handleItem(this, item);
            } else {
                // 如果无法直接输出到核心，则使用原有的卸载逻辑
                super.offload(item);
            }
        }
        @Override
        public boolean dump(Item item) {
            if (!block.hasItems || items.total() == 0 || (item != null && !items.has(item))) {
                return false;
            }
            ...
            CoreBlock.CoreBuild core = Vars.state.teams.get(team()).core();
            if (core != null) {
                Item outputItem = item == null ? items.first() : item;
                if (core.acceptItem(this, outputItem)) {
        ...
        /** 在选中已存在方块时调用，即玩家点击已建造的方块时 */
        public void drawSelect(){
            Drawf.dashCircle(x, y, range, Color.green);              // 显示最大采集范围
            Drawf.dashCircle(x, y, closeDst(), Color.red);           // 显示最小采集范围
            checkOre(tile.x, tile.y, true, null, null);// 显示可开采资源
        }
