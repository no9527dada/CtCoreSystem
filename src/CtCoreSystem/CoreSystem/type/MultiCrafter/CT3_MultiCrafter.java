...
import CtCoreSystem.CoreSystem.draw.CreatorsNumberValue;
...
public class CT3_MultiCrafter extends GenericCrafter {
    ...
    public CT3_MultiCrafter(String name, CreatorsRecipe[] recs) {
        super(name);
        this.recs = recs;
        saveConfig = true;
        configurable = true;
    ...
    public CT3_MultiCrafter(String name, int recLen) {
    ...
    public void setBars() {
        ...
        if (powerBarO) {
            addBar("poweroutput", (CT3_MultiCrafter.ChooseMultiCrafterBuild entity) ->
        ...
        addBar("efficiency", (ChooseMultiCrafterBuild entity) ->
                new Bar(
    ...
    public boolean allRecShow = false;/*是否显示全部合成表*/
    ...
    public class ChooseMultiCrafterBuild extends GenericCrafterBuild {
        ...
        @Override
        public void buildConfiguration(Table table) {//科技树未解锁时该物品不可选
            ...
            for (var i = 0; i < recs.length; i++) {
                ...
                TextureRegion icon;
                if (ItemLiquid == null) {
                    content = recs[i].output.items.length > 0 ? recs[i].output.items[0].item : recs[i].output.liquids[0].liquid;
                    ...
                    content = ItemLiquid[finalI] ? recs[i].output.items[0].item : recs[i].output.liquids[0].liquid;
                    icon = ItemLiquid[finalI] ?
                            recs[i].output.items[0].item.uiIcon : recs[i].output.liquids[0].liquid.uiIcon;
                }
                if (content == null || !content.unlockedNow()) {
                    continue;
                }
        ...
        public float efficiencyMultiplier() {
            return baseEfficiency + Math.min(maxBoost, boostScale * this.attrsum) + attribute.env();
        }
        ...
        public void onProximityUpdate() {
            ...
            this.attrsum = sumAttribute(attribute, this.tile.x, this.tile.y);
        }
