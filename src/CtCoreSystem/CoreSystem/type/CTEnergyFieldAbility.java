...
public class CTEnergyFieldAbility extends Ability {
    ...
    // 修改：将百分比治疗改为具体数值治疗
    public float healAmount = 100f; // 新增：具体治疗数值
    public float healPercent = 0; // 将百分比设为0，禁用百分比治疗
    ...
    CTEnergyFieldAbility() {}

    public CTEnergyFieldAbility(float damage, float reload, float range) {
        this.damage = damage;
        this.reload = reload;
        this.range = range;
        // 默认设置具体治疗数值为100
        this.healAmount = 100f;
    }

    // 新增：支持直接设置治疗数值的构造函数
    public CTEnergyFieldAbility(float damage, float reload, float range, float healAmount) {
        this.damage = damage;
        this.reload = reload;
        this.range = range;
    ...
    public void draw(Unit unit) {
        ...
        Lines.stroke((1.7f + Mathf.absin(blinkScl, 0.7f)), color);
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (((Teamc) other).team() == unit.team) {
                    if (other.damaged()) {
                        ...
                        // 修改：使用具体数值治疗而不是百分比
                        other.heal(healAmount * healMult);
                        healEffect.at(other);
