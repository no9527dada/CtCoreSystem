...
public class CtContentInfoDialog extends ContentInfoDialog {

    public CtContentInfoDialog(){
        super();
        addCloseButton();
        keyDown(key -> {
    ...
    public void show(UnlockableContent content){
        ...
        table.table(title1 -> {
            //实现物品名称的动画文本效果
            title1.add(new AnimatedStatLabels.StatLabelNO(content.localizedName, 0.1f, content, settings.getBool("console") ? "\n[gray]" + content.name : "",5)).width(400f).padLeft(5);
         ...
         /*   title1.image(content.uiIcon).size(iconXLarge).scaling(Scaling.fit);
            ...
            title1.add(new AnimatedStatLabels.StatLabelNO("[accent]" + content.localizedName + (settings.getBool("console") ? "\n[gray]" + content.name : ""), 0.1f)).width(400f).padLeft(5);*/
        });
        ...
        if(content.description != null){
            ...
            //实现物品描述的动画文本效果
            table.add(new AnimatedStatLabels.StatLabel("[lightgray]" + content.displayDescription(),0.5f)).wrap().fillX().padLeft(any ? 10 : 0).width(500f).padTop(any ? 0 : 10).left();
            table.row();
